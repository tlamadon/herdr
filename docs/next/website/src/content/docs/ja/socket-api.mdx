...
`pane.stream_output` は、1 つの接続を単一ペインのライブ端末出力(子プロセスが生成した生の PTY バイト列)に購読させます。フレームをクライアント側の端末エミュレーターにそのまま流し込めば、完全な忠実度で描画できます。ワンショットのスナップショットには引き続き `pane.read` を使ってください。省略可能な `replay` パラメーターは、最初に現在の表示画面を受け取る `"screen"`(デフォルト)か、以降のライブ出力のみを受け取る `"none"` です。

```json
{"id":"stream_out","method":"pane.stream_output","params":{"pane_id":"w1:p1","replay":"screen"}}
```

その後、クライアントが切断するかペインが終了するまで、接続は 1 行につき 1 つの JSON フレームを運びます。`data` フィールドは base64 です:
...
`repaint` は表示画面を ANSI で表したものです。`output` フレームを適用する前に、そのままエミュレーターへ書き込んでください。複数の接続が同じペインを同時にストリーミングできます。読み取りの遅いクライアントがペインを遅くすることはありません。Herdr はそのクライアントのバックログを破棄して `lag` を送り、続けて新しい `repaint` を送って再同期します。
