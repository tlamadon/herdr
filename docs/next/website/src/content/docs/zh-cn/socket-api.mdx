...
`pane.stream_output` 让一个连接订阅单个窗格的实时终端输出——即子进程产生的原始 PTY 字节。把这些帧直接送入客户端的终端模拟器即可获得完全保真的渲染;一次性快照仍应使用 `pane.read`。可选的 `replay` 参数为 `"screen"`(默认,先接收当前可见屏幕)或 `"none"`(只接收之后的实时输出)。

```json
{"id":"stream_out","method":"pane.stream_output","params":{"pane_id":"w1:p1","replay":"screen"}}
```

之后连接每行携带一个 JSON 帧,直到客户端断开或窗格退出。`data` 字段为 base64:
...
`repaint` 是以 ANSI 表示的可见屏幕;先原样写入模拟器,再应用 `output` 帧。多个连接可以同时流式读取同一个窗格。读取过慢的客户端不会拖慢窗格:Herdr 会丢弃其积压数据、发送 `lag`,并随后发送新的 `repaint` 以重新同步。
